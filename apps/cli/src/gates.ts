...
import { fail, type Gate, type RedactionStamp } from "@investigator/core";
...
/**
 * A proposal is generated by this agent from data it already holds, so nothing sensitive enters
 * it that has not already passed the redaction boundary. The stamp records the policy identity so
 * the artifact carries the same provenance as every other stored byte (ADR-0008).
 */
export function proposalStamp(rt: Runtime): RedactionStamp {
  return rt.redactor.policyStamp();
}
